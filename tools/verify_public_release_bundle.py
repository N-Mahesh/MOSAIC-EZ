...
EXPECTED_FILES = frozenset({
    ".gitattributes", "LICENSE", "README.md", "LICENSES.md", "LITERATURE_SEARCH.md", "requirements-release.txt", "RELEASE_MANIFEST.sha256",
    ...
    "tools/verify_public_release_bundle.py",
...
def verify(root: Path, cifair_meta: Path | None = None) -> list[str]:
    root = root.resolve()
    if not root.is_dir():
...
def main() -> int:
    ...
    try:
        reports = verify(args.bundle, args.cifair_meta)
