...
def run(command: list[str], capture_output: bool = False) -> str:
    completed = subprocess.run(command, check=False, capture_output=capture_output, text=True)
...
def fetch(destination: Path, repository: str) -> Path:
    ...
    observed_commit = run(["git", "-C", str(destination), "rev-parse", "HEAD"], capture_output=True)
    if observed_commit != COMMIT:
        raise ValueError(f"checked-out commit mismatch: {observed_commit}")
