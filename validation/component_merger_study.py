...
def merger_changes(population: int, test: int, left: int, right: int) -> tuple[Fraction, Fraction]:
    if left < 2 or right < 2 or left + right > population:
        raise ValueError("merger sizes must be at least two and fit in the population")
    crossing = (
...
def build_study(cifair: dict[str, object]) -> dict[str, object]:
    ...
    return {
        "schema_version": 1,
        "method": "deterministic size-only greedy merger stress; trajectories are not sharp missing-edge bounds",
        "sign_certificate": "each merger weakly decreases expected crossing-component count and weakly increases expected exposure",
        "datasets": datasets,
    }
